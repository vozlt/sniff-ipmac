...
/* 받아 들일 한패킷의 최대 크기, 한 네트윅 카드면 이정도면 충분하다. */
#define SNAPLEN			65535

/* 0 == 자신에게 오는 패킷만 캡쳐 , 1 == 모든 로컬상으로 오는 패킷 캡쳐 */
#define PROMISC			1

/* 패킷 읽기시 최대 기다림 시간 1/1000단위, 모든 시스템에서 이값을 적용하지는 않는다. 0을 주면 계속 기다린다. */
#define TO_MS			1000	

/* 받을수 있는 data의 최대 크기 지정 8192 */
...
void sniff_packet_ipmac(const u_char *pkt_data)
{
	...
	/*--====
	 * IPPROTO_TCP == 6
	 * iph->ip_len 이길이가 전체 TCP/IP 헤더의 길이이다. TCP(20) + IP(20) + data(...)
	 * ETHERTYPE_IP == 0x0800
	 ====--*/
...
void sniff_packet_verbose(const u_char *pkt_data)
{
	...
	/*--==== 
	 * /usr/include/netinet/tcp.h 여기에 정의된 값 doff * 4 해주었음!
	 * libpcap:sniffex.c 예제의 (((ip)->ip_vhl) & 0x0f) 는 doff 와 같겠지?
	 ====--*/
